...
class PluginUpdater
{
    ...
    private function plugin_update_message()
    {
        ...
        else
        {
            ...
            if( isset($transientCache['updates']) && ! empty($transientCache['updates']))
            {
                foreach (Bootstrap::$addons as $addon)
                {
                    $slug = $addon->getAddonSlug();
                    $currentVersion = $addon->getVersion();
                    if( isset($transientCache['updates'][$slug]) && isset($transientCache['updates'][$slug]['version']) && $transientCache['updates'][$slug]['version'] != $currentVersion )
                    {
                        $showNotice = true;
                        break;
                    }
