...
//		}
	...
	public function getAllTimeslots()
	{
        ...
        if( $this->isRecurring() && empty($this->timeslots) && empty( $this->recurringAppointmentsList ) )
        {
	...
	public function validateAppointmentsAvailability()
	{
		...
		foreach ($this->getAllTimeslots() AS $timeSlot )
		{
			if( ! $timeSlot->isBookable() )
			{
				throw new \Exception( bkntc__('Please select a valid time! ( %s %s is busy! )', [ Date::datee( $timeSlot->getDate(), false, true ), Date::time( $timeSlot->getTime(), false, true ) ]) );
			}
		}
	}
