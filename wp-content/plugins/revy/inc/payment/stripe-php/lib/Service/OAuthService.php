...
class OAuthService extends AbstractService
{
     ...
     *
     * @return StripeObject the object returned by Stripe's Connect API
     */
    ...
    public function authorizeUrl($params = null, $opts = null)
    {
        ...
        $query = Util::encodeParameters($params);
     ...
     * @param null|array $opts
     *
     * @throws OAuthErrorException if the request fails
     *
    ...
    private function _parseOpts($opts)
    {
        if (is_array($opts)) {
            if (array_key_exists('connect_base', $opts)) {
                ...
                throw new InvalidArgumentException('Use `api_base`, not `connect_base`');
            }
        }

        return RequestOptions::parse($opts);
    }
